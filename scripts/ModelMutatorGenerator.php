...
class ModelMutatorGenerator {
  ...
  private function getDeleteMethod(): CodegenMethod {
    return
      codegen_method('delete')
        ->addParameter('int $id')
        ->setReturnType('void')
        ->setIsStatic()
        ->setBody(
          hack_builder()->addLine(
            'DB::delete("%s", "%s=%%s", $id);',
  ...
  private function getSaveMethod(): CodegenMethod {
    $body = hack_builder()->addAssignment('$id', '$this->id')->startIfBlock(
      '$id === null',
    )->addLine('$this->checkRequiredFields();')->addLine(
      'DB::insert("%s", $this->data->toArray());',
      $this->schema->getTableName(),
    )->addReturn('(int) DB::insertId()')->addElseBlock()->addLine(
      'DB::update("%s", $this->data->toArray(), "%s=%%s", $this->id);',
      $this->schema->getTableName(),
      $this->schema->getIdField(),
    )->addReturn('$id')->endIfBlock();
