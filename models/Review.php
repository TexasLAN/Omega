...
final class Review {
  ...
  public static function loadByUserAndApp(int $user_id, int $app_id): ?Review {
    $query = DB::queryFirstRow("SELECT * FROM reviews WHERE user_id=%s AND application_id=%s", $user_id, $app_id);
    if(!$query) {
      return null;
    }
  ...
  public static function loadByApp(int $app_id): array<Review> {
    $query = DB::query("SELECT * FROM reviews WHERE application_id=%s", $app_id);
    if(!$query) {
      return array();
    }
  ...
  public static function getAppCount(int $app_id): int {
    $query = DB::queryFirstRow("SELECT COUNT(*) FROM reviews WHERE application_id=%s", $app_id);
